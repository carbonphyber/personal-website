  ...
  return (
    <div className="container">
      ...
      {dogsArray.map((dog) => (
        <div class="row">
          <figure
            class={`jumbotron rescue-dog rescue-dog-${dog.id} ${dog.imageUrl ? '' : 'no-image'}`}
            id={dog.id}
            style={{
              backgroundImage: `url(${dog.imageUrl})` || '',
            }}
          >
            <figcaption class={`rescue-dog--caption rescue-dog--caption__${dog.captionAlign || 'left'}`}>
              <h2><a href={`#${dog.id}`} className="rescue-dog--header">{dog.name}</a></h2>
              <h3 class="rescue-dog--breed">{dog.breedGuess || ''}</h3>
              <h3 class="rescue-dog--vitals">{[dog.ageGuess, dog.weight].join(', ')}</h3>
              <p>
                <span class="label label-info">{`Rescued ${dog.dateRescued}`}</span>
                &nbsp;
                {dog.dateAdopted && <span class="label label-success">{`Adopted ${dog.dateAdopted}`}</span>}
              </p>
            </figcaption>
